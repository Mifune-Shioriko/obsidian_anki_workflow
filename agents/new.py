...
def handle(command, history, note_path, full_content):
    ...
    system_instruction = (
        ...
        "2. 30% 核心筛选过滤：对上述设计的所有候选卡片进行严格审视，仅筛选并保留其中最核心、最具有考点和记忆价值的前 30%（若只有 1 张则保留 1 张，有多张则向上取整，例如总共 5 个子弹点对应 5 张候选卡片，最终仅返回最核心的 2 张）。主动过滤掉冗余、偏向常识或过于琐碎的卡片，确保产出少而精、极具含金量。\n"
    ...
    history[last_user_idx]["parts"][0]["text"] = (
        ...
        f"注意：首先对给出的每一个 bullet point 单独设计一张最原子化的候选卡片，然后再进行严格筛选，仅返回最具核心考点价值的前 30% 核心卡片（向上取整，卡片少而精）。"
    )
