...
def handle(command, history, note_path, full_content):
    ...
    system_instruction = (
        "你是一个极其严谨且具备深厚教学诊断能力的 Anki 卡片制作专家。你的任务是根据给出的【笔记正文内容】和先前的【讨论历史】，提炼出高度原子化且最具核心考点价值的新卡片。\n\n"
        "【双重筛选制卡决策模型（单次完成）】\n"
        "你不能自主无差别建卡。你必须针对当前讨论的知识建立高质量卡片：\n"
        "1. 脑海中原子化拆分：首先对整篇材料中的核心概念和关键点进行虚拟拆分，列出所有可能的最微小、单一的考点。\n"
        "2. 50% 核心筛选过滤：在输出前进行严格筛选，仅保留其中最核心、最具考点价值的前 50% 的卡片（若只有 1 个考点则保留 1 个，多个则向上取整）。主动过滤掉冗余、次要、过于琐碎或容易由常识推导出的考点，确保卡片少而精、极具含金量。\n"
        "3. 严禁超纲：严禁针对当前给出的笔记范围或讨论之外的任何背景信息制作卡片。\n\n"
        "【卡片生成黄金规则（必遵守）】\n"
    ...
    history[last_user_idx]["parts"][0]["text"] = (
        f"{original_text}\n\n[系统注入] 请直接根据当前的笔记正文以及上述讨论历史，根据【双重筛选制卡决策模型（单次完成）】生成最核心的高价值全新原子化卡片。所有新卡片的 id 必须为空字符串 \"\"。\n"
        f"注意：必须在脑海中充分拆分，并在输出前严格过滤筛选，仅返回最核心的前 50% 核心卡片（向上取整，卡片少而精）。"
    )
