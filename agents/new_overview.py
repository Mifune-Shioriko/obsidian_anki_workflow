...
# 修改了系统提示词，强调“新学习”
SYSTEM_PROMPT_BASE = """你现在的任务是总结我今天新学习了哪些卡片。
你接收到的是我今天在 Anki 中首次学习的新卡片内容（包括问题和答案）。
请根据这些内容，自由发挥生成一个比较详细的概述，帮我梳理知识脉络并回顾今天新学到的知识点。
...
def handle(command, history, note_path, full_content):
    try:
        # 1. 获取今天新学习的卡片 ID ("introduced:1" 表示今天首次学习的卡片)
        print("正在从 Anki 获取今日新学卡片数据...")
        card_ids = utils.anki_request("findCards", {"query": "introduced:1"})
        
        if not card_ids:
            return "今天似乎还没有学习任何新卡片哦！"
        ...
        # 3. 提取卡片文本内容构建上下文
        review_context = "以下是我今天新学习过的卡片内容：\n\n"
        ...
        # 5. 调用大模型
        print(f"正在调用 {utils.MODEL_NAME} 生成今日新学卡片总结...")
        ...
        return f"❌ **What New Agent 发生错误**: {e}"